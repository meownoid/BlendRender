...
RUN apt-get update && apt-get install -y --no-install-recommends \
      ca-certificates curl xz-utils tini python3 util-linux libgomp1 libimath-3-1-29t64 \
...
COPY --from=flip-fluids-builder /build/build/bl_flip_fluids/flip_fluids_addon/ \
    /opt/blender/5.2/scripts/addons_core/flip_fluids_addon/
COPY --from=alembic-builder /opt/alembic/lib/ \
    /opt/blender/5.2/scripts/addons_core/flip_fluids_addon/ffengine/lib/
...
RUN groupadd --gid 10001 blendrender \
    ...
    && chown -R blendrender:blendrender \
        /opt/blender/5.2/scripts/addons_core/flip_fluids_addon/resources/installation_data
