...
ENV DEBIAN_FRONTEND=noninteractive \
    ...
    PYTHONUNBUFFERED=1 \
    DATA_ROOT=/var/lib/blendqueue \
    BLENDER_BIN=/opt/blender/blender \
    RENDERER_SCRIPT=/app/renderer/blendqueue_render.py \
    FRONTEND_DIST=/app/frontend/dist

RUN apt-get update && apt-get install -y --no-install-recommends \
      ca-certificates curl xz-utils tini python3 \
...
RUN groupadd --gid 10001 blendqueue \
    && useradd --uid 10001 --gid blendqueue --no-create-home --shell /usr/sbin/nologin blendqueue \
    && mkdir -p /var/lib/blendqueue \
    && chown -R blendqueue:blendqueue /var/lib/blendqueue /app
...
CMD ["uvicorn", "blendqueue.main:app", "--host", "0.0.0.0", "--port", "8000", "--workers", "1", "--proxy-headers", "--forwarded-allow-ips", "*"]
