...
async def test_existing_database_adds_optional_render_and_frame_telemetry_columns(settings) -> None:
...
async def test_frame_sample_telemetry_is_persisted_and_cleared_on_requeue(settings) -> None:
    database = Database(settings.database_path)
    await database.initialize()
    job = await database.create_job(
    ...
    updated = await database.update(job.id, sample_current=16, sample_total=64)
    assert (updated.sample_current, updated.sample_total) == (16, 64)

    requeued = await database.requeue(job.id, [])
    assert (requeued.sample_current, requeued.sample_total) == (None, None)
