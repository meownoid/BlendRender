...
export function NewJobPanel({ open, scene, system, busy, onClose, onSubmit }: NewJobPanelProps) {
  ...
  return <aside className="side-panel" aria-label="New render job">
    ...
    {mode === 'still' ? <label className="field"><span>Frame</span><input type="number" value={frame} onChange={(event) => setFrame(event.target.value)} onBlur={(event) => validateFrameOnBlur(event.target.value, 'Frame')} /></label> : <div className="range-fields"><label className="field"><span>Start</span><input type="number" value={start} onChange={(event) => setStart(event.target.value)} onBlur={(event) => validateFrameOnBlur(event.target.value, 'Start frame')} /></label><label className="field"><span>End</span><input type="number" value={end} onChange={(event) => setEnd(event.target.value)} onBlur={(event) => validateFrameOnBlur(event.target.value, 'End frame')} /></label></div>}
