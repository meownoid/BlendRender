...
export function SceneWorkspace({ scene, frames, jobs, onDelete }: SceneWorkspaceProps) {
  ...
  return <main className="scene-workspace">
    <section className="scene-heading"><div><h1>{scene.filename}</h1><p>{scene.source_kind === 'zip' ? 'Project archive' : 'Blend file'} · {scene.result_count} published results · {active.length} active jobs</p></div><button className="button button--danger" onClick={() => void onDelete(scene)}><Trash2 size={16} /> Delete scene</button></section>
